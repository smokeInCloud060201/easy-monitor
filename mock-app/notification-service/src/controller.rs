...
pub async fn handle_notify(
    ...
    match service.process_notification(body.into_inner()).await {
        Ok(res) => HttpResponse::Ok().json(res),
        Err(e) => {
            if e == "SMTP timeout" {
                HttpResponse::GatewayTimeout().json(NotifyResponse {
                    status: "timeout".to_string(),
                    notification_id: "none".to_string(),
                    order_id,
...
pub async fn handle_get_notification(
    ...
    if let Some(status) = service.get_status(&order_id).await {
        ...
        HttpResponse::NotFound().json(serde_json::json!({"error": "Notification not found"}))
    }
}
...
pub async fn handle_health() -> HttpResponse {
    HttpResponse::Ok().json(HealthResponse {
        ...
        service: "notification-service (DDD)".to_string(),
    })
}
