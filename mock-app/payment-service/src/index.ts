...
async function simulateSpan(name: string, minMs: number, maxMs: number, attrs: Record<string, string> = {}): Promise<void> {
  const span = tracer.startSpan(name, {}, otelContext.active());
  try {
    Object.entries(attrs).forEach(([k, v]) => span.setAttribute(k, v));
    await Bun.sleep(randomMs(minMs, maxMs));
...
async function simulateDbSpan(op: string, table: string, statement: string, minMs: number, maxMs: number): Promise<void> {
  const span = tracer.startSpan(`db.query ${op} ${table}`, {}, otelContext.active());
  try {
...
async function simulateCacheSpan(op: string, key: string): Promise<boolean> {
  const hit = Math.random() < 0.7;
  const span = tracer.startSpan(`cache.${op} ${key}`, {}, otelContext.active());
  try {
    span.setAttribute('cache.system', 'redis');
    span.setAttribute('cache.operation', op);
    span.setAttribute('cache.key', key);
    span.setAttribute('cache.hit', hit);
    await Bun.sleep(hit ? randomMs(1, 5) : randomMs(2, 8));
    span.setStatus({ code: SpanStatusCode.OK });
  } finally {
    span.end();
  }
  ...
  async fetch(req) {
    ...
    if (req.method === 'POST' && url.pathname === '/api/charge') {
      ...
      try {
        ...
        await simulateSpan('validate_card', 10, 30, {
        ...
        await simulateSpan('fraud_check', 30, 120, {
        ...
        if (isSuccess) {
          ...
          await simulateCacheSpan('SET', 'balance:merchant_001');
    ...
    if (req.method === 'GET' && url.pathname.startsWith('/api/payment/status/')) {
      ...
      const cacheHit = await simulateCacheSpan('GET', `txn:${id}`);
      if (!cacheHit) {
        await simulateDbSpan('SELECT', 'transactions',
